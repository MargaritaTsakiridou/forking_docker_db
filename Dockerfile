...
COPY --chown=deployment:deployment requirements.txt .
RUN /databricks/python3/bin/pip install --no-cache-dir -r requirements.txt

# Copy script with correct ownership
COPY --chown=deployment:deployment app.py .
